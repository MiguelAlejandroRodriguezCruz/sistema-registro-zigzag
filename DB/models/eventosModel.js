...
const eventosModel = {
  ...
  obtenerRegistradosPorVisitante: async (idVisitante) => {
    const query = `
            SELECT e.*
            FROM evento e
            WHERE e.id IN (
  ...
  obtenerBanner: async (id) => {
    const [rows] = await db
      .promise()
      .query("SELECT baner FROM evento WHERE id = ?", [id]);
    return rows;
  },
  ...
  insertarImagen: async (idEvento, ruta) => {
    await db
      .promise()
      .query("INSERT INTO imagenes (evento_id, ruta_imagen) VALUES (?, ?)", [
        idEvento,
        ruta,
      ]);
  },
  ...
  obtenerImagenesPorEvento: async (idEvento) => {
    const [rows] = await db
      .promise()
      .query("SELECT id, ruta_imagen FROM imagenes WHERE evento_id = ?", [
        idEvento,
      ]);
    return rows;
  },
  ...
  obtenerImagenPorId: async (idImagen) => {
    const [rows] = await db
      .promise()
      .query("SELECT ruta_imagen FROM imagenes WHERE id = ?", [idImagen]);
    return rows;
  },
