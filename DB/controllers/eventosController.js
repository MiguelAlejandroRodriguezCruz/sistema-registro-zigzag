...
const eventosController = {
  ...
  getNoRegistrados: async (req, res) => {
    try {
      const eventos = await eventosModel.obtenerNoRegistradosPorVisitante(
        req.params.idVisitante,
      );
      res.json(eventos);
    } catch (err) {
  ...
  getRegistrados: async (req, res) => {
    try {
      const eventos = await eventosModel.obtenerRegistradosPorVisitante(
        req.params.idVisitante,
      );
      res.json(eventos);
    } catch (err) {
      console.error("Error al obtener eventos registrados:", err.message);
      res.status(500).json({ message: "Error interno del servidor" });
    }
  },
  ...
  crear: async (req, res) => {
    try {
      ...
      if (!req.file) {
        return res
          .status(400)
          .json({ message: "Debe subir un banner para el evento" });
      }
  ...
  subirImagenesEvento: async (req, res) => {
    ...
    try {
      ...
      res
        .status(201)
        .json({ message: "Imágenes subidas correctamente", imagenes });
    } catch (err) {
